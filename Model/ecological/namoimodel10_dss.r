...
# setwd("~/Dropbox/integrated/Baihua/Model_dss")
setwd("~/Dropbox/integrated/Mike/ecological")
...
for (i in 1:length(scenariolist)){
  ...
  for (j in 1:length(asset.table[,1])){
        ...
        outfile = paste("Outputs/", scenariolist[i],"/", today, "_asset", j, ".csv", sep = "")
        write.zoo(index.all, file=outfile,sep=",")
  }
  }
